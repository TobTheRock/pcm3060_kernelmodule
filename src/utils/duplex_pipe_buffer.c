...
typedef struct _duplex_duplex_pipe_buffer_impl
{
    pipe_buffer_t *buffer_a, *buffer_b;
} _duplex_duplex_pipe_buffer_impl_t;
...
static _duplex_duplex_pipe_buffer_impl_t* _get_duplex_pipe_buffer_impl(const unsigned int size)
{
    _duplex_duplex_pipe_buffer_impl_t* newbuf;
    TRACE("");
    if ((newbuf = kmalloc(sizeof(_duplex_duplex_pipe_buffer_impl_t), GFP_KERNEL)) == NULL)
    {
...
static void _put_duplex_pipe_buffer_impl(_duplex_duplex_pipe_buffer_impl_t* bufimpl)
{
    TRACE("");
...
duplex_pipe_buffer_t* get_duplex_pipe_buffer(const unsigned int size)
{
    ...
    else if ((impl = _get_duplex_pipe_buffer_impl(size)) == NULL)
    {
    ...
    r_buf_impl:
        _put_duplex_pipe_buffer_impl(impl);
    r_buf:
        kfree(newbuf);
    r_null:
        return NULL;
...
void put_duplex_pipe_buffer(duplex_pipe_buffer_t* buf)
{
    ...
    _put_duplex_pipe_buffer_impl(buf->_impl_p);
    kfree(buf);
}
...
unsigned int duplex_pipe_end_copy_from_user(struct duplex_pipe_end* duplex_pipe_end, const void* buffer_ext, const unsigned int buflen)
{
    ...
    return pipe_buffer_copy_from_user(duplex_pipe_end->write_buffer);
}
...
unsigned int duplex_pipe_end_copy_to_user(const struct duplex_pipe_end* duplex_pipe_end, void* buffer_ext, const unsigned int buflen, const unsigned int off)
{
    TRACE("");
    RETURN_ON_NULL(duplex_pipe_end, buflen);
    return pipe_buffer_copy_to_user(duplex_pipe_end->read_buffer);
...
{
    RETURN_ON_NULL(duplex_pipe_end, 0);
    return pipe_buffer_n_bytes_available(duplex_pipe_end->read_buffer, o_buffer_ptr);
}
...
void duplex_pipe_end_read_end(const duplex_pipe_end_t* duplex_pipe_end)
{
    ...
    return pipe_buffer_read_end(duplex_pipe_end->read_buffer, o_buffer_ptr);
}
...
{
    RETURN_ON_NULL(duplex_pipe_end, 0);
    return pipe_buffer_write_start(duplex_pipe_end->read_buffer, o_buffer_ptr);
}

void duplex_pipe_end_write_end(duplex_pipe_end_t* duplex_pipe_end)
{
    RETURN_VOID_ON_NULL(duplex_pipe_end);
    return pipe_buffer_write_end(duplex_pipe_end->read_buffer, o_buffer_ptr);
}
